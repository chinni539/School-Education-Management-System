// server.js  —  EduCore Backend (PostgreSQL / Supabase edition)
'use strict';
...
app.get('/health', async (_req, res) => {
  ...
  try {
    ...
    res.json({ status: 'ok', db: 'connected', db_time: r.rows[0].db_time, service: 'EduCore API (PostgreSQL)' });
  } catch {
...
async function start() {
  try {
    await testConnection();   // Verify DB is reachable before opening port
    app.listen(PORT, () => {
      console.log(`\n🚀  EduCore API  →  http://localhost:${PORT}`);
