// routes/fees.js  — Updated for EduCore v2 (fee breakdown + studentId filter)
'use strict';
...
router.get('/', async (req, res, next) => {
  try {
    const {
      studentId, status, feeType, academicYear,
      page = 1, limit = 50
    } = req.query;
    ...
    const result = await query(
      `SELECT f.receipt_no, f.student_id,
              ...
              f.payment_mode, f.status, f.remarks,
              f.academic_year,
              f.created_at
       FROM   fees f
...
router.post('/', async (req, res, next) => {
  try {
    const {
      ...
      paymentMode, paidDate, remarks, academicYear
    } = req.body;
    ...
    const result = await query(
      `INSERT INTO fees
         (student_id, fee_type, amount, paid_date, payment_mode, status, remarks, academic_year)
